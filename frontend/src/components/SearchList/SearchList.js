...
const SearchList = ({movieList, page}) => {
    return (
        <div>
            {movieList && movieList[page] && movieList[page]?.search?.length && movieList[page]?.search.map(movie => <Link to={`/film/?${movie.id}`}><Image className="ml-5" src={movie.medium_cover_image} /></Link>)}
